<template>
    <div class="border border-gray-200 p-3 mb-4 rounded">
        ...
        <div v-show="showForm">
            <div v-if="show_alert" :class="alert_variant" class="text-white text-center font-bold p-4 mb-4">
                {{ alert_message }}
            </div>
            <vee-form :validation-schema="schema" :initial-values="song" @submit="edit">
                ...
                <div class="mb-3">
                    ...
                    <ErrorMessage class="text-red-600 block" name="genre" />
                </div>
                <button :disabled="in_submission" type="submit" class="py-1.5 px-3 rounded text-white bg-green-600 mx-1">
                    Submit
                </button>
                <button @click.prevent="showForm = false" :disabled="in_submission" type="button"
                    class="py-1.5 px-3 rounded text-white bg-gray-600 mx-1">
                    Go Back
                </button>
            </vee-form>
        </div>
...
export default {
    ...
    methods: {
        ...
        async deleteSong() {
            ...
            const songRef = storageRef.child(`songs/${this.song.oroginal_name}`);
